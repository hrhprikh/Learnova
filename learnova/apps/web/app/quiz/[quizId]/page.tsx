            ...
            <>
              ...
              <div className="flex flex-col gap-4">
                {current.options.map((option) => (
                  <button
                    key={option.id}
                    className={`text-left w-full p-6 rounded-2xl border bg-white hover:border-[var(--ink)] hover:shadow-[0_10px_30px_-15px_rgba(26,28,41,0.1)] transition-all duration-300 flex items-center justify-between group ${selected === option.id ? "border-2 border-[var(--ink)] bg-[var(--accent-blue)]/5" : "border-[var(--edge)]"}`}
                  ...
                  >
                    ...
                    <div className={`w-6 h-6 rounded border flex items-center justify-center shrink-0 ml-6 transition-colors ${selected === option.id ? "border-2 border-[var(--ink)] bg-[var(--ink)]" : "border-[var(--edge)] group-hover:border-[var(--ink)]"}`}>
                      {selected === option.id ? <div className="w-2.5 h-2.5 bg-white rounded-sm" /> : null}
                    </div>
                  </button>
                ))}
              </div>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    const nextAnswers = [
                      ...
                      { questionId: current.id, optionId: selected }
                    ];
