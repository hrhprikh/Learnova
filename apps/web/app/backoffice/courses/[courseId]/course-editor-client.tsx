...
export default function InstructorCourseEditor({ params }: { params: { courseId: string } }) {
    ...
    const [didAutoOpenLesson, setDidAutoOpenLesson] = useState(false);
    ...
    useEffect(() => {
        const shouldAutoOpen = searchParams.get("openLesson") === "1";
        if (!course || didAutoOpenLesson || !shouldAutoOpen) return;
        openAddLesson();
    ...
    async function togglePublish() {
        ...
        try {
            const result = await apiRequest<{ course: { published: boolean } }>(`/courses/${params.courseId}/${course.published ? "unpublish" : "publish"}`, {
                method: "POST", token
            });
            setCourse((prev) => {
                if (!prev) return prev;
                return { ...prev, published: result.course.published };
            });
        } catch (err) {
            setPublishError(err instanceof Error ? err.message : "Failed to change publish state");
        } finally {
    ...
    return (
        <div className="min-h-screen bg-[var(--bg)] pb-24">
            <header className="px-6 py-5 lg:px-12 flex items-center justify-between border-b border-[var(--edge)] bg-white/50 backdrop-blur-md sticky top-0 z-40">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 text-xs font-bold px-4 py-2 rounded-xl transition-colors disabled:opacity-60 ${course.published ? 'bg-green-50 text-green-700 border border-green-200 hover:bg-green-100' : 'bg-orange-50 text-orange-700 border border-orange-200 hover:bg-orange-100'}`}
                    >
                        {isTogglingPublish ? "Updating..." : course.published ? "✓ Published" : "Publish"}
                    </button>
            ...
            {publishError ? <p className="px-6 lg:px-12 mt-3 text-sm text-red-600">{publishError}</p> : null}
                            ...
                            {activeTab === "CONTENT" && (
                                <div className="max-w-2xl mx-auto space-y-8">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <textarea 
                                            rows={8}
                                            className="w-full p-5 rounded-2xl border border-[var(--edge)] bg-white outline-none focus:border-[var(--ink)] transition-colors leading-relaxed"
                                    ...
                                    <div className="bg-white rounded-3xl border border-[var(--edge)] p-8">
                                        ...
                                        <p className="text-sm text-[var(--ink-soft)] mb-6">Attach extra reading, code snippets, or asset files here.</p>
